...
const NCISM = () => {
  ...
  const fetchNcismData = async () => {
    try {
      setIsLoading(true);
      const res = await fetch("/api/ncism");
      ...
      const transformed = data.map(tab => ({
        ...
        sections: (tab.sections || []).map(section => ({
          ...
          files: (section.pdfs || []).map(pdf => ({
            id: pdf.id,
            name: pdf.name,
            url: pdf.pdf_path, // map pdf_path → url
            uploadDate: pdf.uploadedDate,
          }))
        }))
      }));
  ...
  const handleConfirmDelete = async () => {
    ...
    if (type === 'TAB') {
      try {
        await fetch(`/api/ncism/${tabId}`, { method: 'DELETE' });

...
// ✅ Safe Image URL Combiner Helper
...
const UpcomingEvents = () => {
  ...
  const confirmDelete = async () => {
    try {
      const res = await fetch(
        `${BACKEND_URL}/api/events/${deleteConfirm}`,
        {
          method: "DELETE",
        },
      );
  ...
  const filteredEvents = events.filter((event) => {
    if (yearFilter !== "all") {
      return new Date(event.eventDate).getFullYear().toString() === yearFilter;
    }
    return true;
  });

  const availableYears = Array.from(
    new Set(events.map((e) => new Date(e.eventDate).getFullYear().toString())),
  ).sort((a, b) => b - a);
  ...
  const EventCard = ({ event }) => {
    const imageSrc = getFullImageUrl(event.coverImage);
    ...
    return (
      <div className="bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden w-60 h-full flex flex-col group border border-gray-200">
        <div className="relative overflow-hidden pt-[60%] w-full bg-gray-100">
          {imageSrc ? (
            <img
              src={imageSrc}
              alt={event.title}
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center relative">
                {formData.coverImagePreview ? (
                  <div className="mb-3 relative">
                    <img
                      src={getFullImageUrl(formData.coverImagePreview)}
                      alt="Preview"

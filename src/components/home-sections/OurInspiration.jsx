...
const OurInspiration = () => {
  ...
  // 🌟 BULLETPROOF HELPER TO FIX IMAGE URLS (Legacy & New) 🌟
  const getFullImageUrl = (path) => {
    if (!path) return null;
    
    // Agar base64 preview, blob ya pura URL hai toh waise hi rehne do
    ...
    // 🔥 MAGIC FIX: Purane database entries (/assets/) ko automatically (/uploads/) kar do
  ...
  const fetchData = async () => {
    try {
      ...
      setInspiration({
        ...
        image: data.inspiration?.imageUrl || null,
      });
      ...
      setPrincipal({
        ...
        image: data.principal?.imageUrl || null,
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `${BACKEND_URL}/api/authorities${endpoint}`,
        {
          method: "POST",
          body: apiData,
        },
  ...
  const confirmDelete = async () => {
    if (deleteConfirm) {
      try {
        await fetch(
          `${BACKEND_URL}/api/authorities/pillar/${deleteConfirm}`,
          {
            method: "DELETE",
          },
        );
            ...
            {activeTab === "inspiration" && (
              <div className="flex flex-col items-center">
                ...
                <div className="w-60 h-72 rounded-3xl shadow-2xl overflow-hidden mb-8 bg-gray-200 flex items-center justify-center">
                  {inspiration.image ? (
                    <img
                      src={getFullImageUrl(inspiration.image)}
              ...
              <div className="flex flex-col items-center">
                ...
                <div className="mx-auto flex rounded-2xl w-full max-w-[500px] overflow-hidden shadow-lg h-48 bg-white border border-gray-100 relative">
                  <div className="w-40 flex-shrink-0 bg-gray-100 flex items-center justify-center">
                    {principal.image ? (
                      <img
                        src={getFullImageUrl(principal.image)}
                        className="w-full h-full object-cover"
                        alt="principal"
                      />
                    ) : (

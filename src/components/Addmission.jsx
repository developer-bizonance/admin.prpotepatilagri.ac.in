...
const Addmission = () => {
  ...
  const saveChanges = async () => {
    ...
    try {
      let url = "http://localhost:4001/api/admissions";
      let method = "POST";

      if (editingId) {
        url = `http://localhost:4001/api/admissions/${editingId}`;
        method = "PUT";
      }
  ...
  const handleConfirmDelete = async () => {
    if (!deleteId) return;
    try {
      await fetch(`http://localhost:4001/api/admissions/${deleteId}`, { method: "DELETE" });
      await fetchAdmissions();
    } catch (error) {
            ...
            <div className="fixed inset-0 z-50 flex justify-end">
              ...
              >
                ...
                <div className="p-6 flex-1 overflow-y-auto bg-slate-50">
                  ...
                  {/* List of Files matching the active category */}

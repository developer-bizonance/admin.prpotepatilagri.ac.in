...
import CardManager from "./Chairman.jsx";
import PrincipalManager from "./PrincipalDetails.jsx";
import Addmission from "./Addmission.jsx";
import UniversityDetails from "./UniversityDetails.jsx";
...
import FacultyAdmin from "./home-sections/FacultyAdmin.jsx";
import Student from "./Student.jsx";
import GoverningBodies from "./GoverningBodies.jsx";
import FacilitiesAdmin from "./FacilitiesAdmin.jsx";
import GalleryAdmin from "./EventGalleryAdmin.jsx"; 
import PopupEventsDashboard from "./PopupEventsDashboard.jsx"; 
...
const sidebarItems = [
  { title: "Home", Icon: HomeIcon, Content: Home },
  {
    ...
    Icon: Info,
    children: [
      { title: "Administration Desk", Icon: User, Content: CardManager },
      { title: "Principal Details", Icon: User, Content: PrincipalManager },
      { title: "Governing Bodies", Icon: Users, Content: GoverningBodies },
    ],
  },
  { title: "Addmission", Icon: FileCheck, Content: Addmission },
  { title: "Faculty", Icon: User, Content: FacultyAdmin },
  { title: "Student", Icon: User, Content: Student },
  { title: "Facilities", Icon: School, Content: FacilitiesAdmin },
  { title: "Event Gallery", Icon: ImageIcon, Content: GalleryAdmin },
  { title: "Popup Banners", Icon: PlusCircle, Content: PopupEventsDashboard },
  { title: "Useful Links", Icon: Link, Content: ImportantLinks },
    ...
    return (
      ...
      >
        ...
        >
          ...
          {isOpen && (
            <>
              <span className="flex-1 font-medium text-left whitespace-nowrap">
                {item.title}
              </span>
              {isGroup && (
                <motion.span
    ...
    return (
      <motion.div key={index} className="flex-shrink-0 px-2">
        ...
        >
          ...
          <span className="text-xs truncate max-w-[4rem]">{item.title}</span>
        </button>
      </motion.div>
    );
  };
    ...
    <div className="relative h-full">
      {!isMobile && (
        <div className="flex h-[calc(100vh-5rem)]">
          <motion.div
            className="flex flex-col bg-white shadow-md pt-8 overflow-y-auto no-scrollbar"
      ...
      {isMobile && (
        <div className="flex flex-col h-full">
          ...
          <motion.div className="fixed bottom-0 left-0 right-0 bg-white border-t shadow-lg z-50">
            <div className="flex overflow-x-auto no-scrollbar">
              {sidebarItems
                .flatMap((i) => (i.children ? i.children : i))
                .map((item, index) => renderMobileTab(item, index))}
            </div>
          </motion.div>
        </div>
      )}
